...
void make(TFile* file, TString fileDir = "BMTF", bool makePng = false, TString run = "1", TString dir = ".") {
  ...
  TH1F* wheelData = (TH1F*)file->Get(fileDir+"/wheelData");
  TH1I* dxyData = (TH1I*)file->Get(fileDir+"/dxyData");
  TH1I* TA1Data = (TH1I*)file->Get(fileDir+"/TA1Data");
  TH1I* TA2Data = (TH1I*)file->Get(fileDir+"/TA2Data");
  TH1I* TA3Data = (TH1I*)file->Get(fileDir+"/TA3Data");
  TH1I* TA4Data = (TH1I*)file->Get(fileDir+"/TA4Data");
  ...
  TH1F* wheelEmu = (TH1F*)file->Get(fileDir+"/wheelEmu");
  TH1I* dxyEmu = (TH1I*)file->Get(fileDir+"/dxyEmu");
  TH1I* TA1Emu = (TH1I*)file->Get(fileDir+"/TA1Emu");
  TH1I* TA2Emu = (TH1I*)file->Get(fileDir+"/TA2Emu");
  TH1I* TA3Emu = (TH1I*)file->Get(fileDir+"/TA3Emu");
  TH1I* TA4Emu = (TH1I*)file->Get(fileDir+"/TA4Emu");
  ...
  TCanvas *canv0 = canvas_data_mc(phisData, phisEmu, "#phi", true, -20, 80, 0, 1000, "", run);
  std::cout << "phi ok" << std::endl;
  TCanvas *canv1 = canvas_data_mc(etasData, etasEmu, "#eta", true, -140, 140, 0, 1000, "", run);
  std::cout << "eta ok" << std::endl;
  TCanvas *canv2 = canvas_data_mc(ptData, ptEmu, "Pt", true, 0, 300, 0, 1000, "", run);
  ...
  TCanvas *canv6 = canvas_data_mc(TA1Data, TA1Emu, "TA1", true, 0, 16, 0, 1000, "", run);
  std::cout << "TA1 ok" << std::endl;
  TCanvas *canv7 = canvas_data_mc(TA2Data, TA2Emu, "TA2", true, 0, 16, 0, 1000, "", run);
  std::cout << "TA2 ok" << std::endl;
  TCanvas *canv8 = canvas_data_mc(TA3Data, TA3Emu, "TA3", true, 0, 16, 0, 1000, "", run);
  std::cout << "TA3 ok" << std::endl;
  TCanvas *canv9 = canvas_data_mc(TA4Data, TA4Emu, "TA4", true, 0, 16, 0, 1000, "", run);//check maxy
  std::cout << "TA4 ok" << std::endl;
  ...
  if (makePng) {
    ...
    img->FromPad(canv4);
    img->WriteImage(dir+"/compare_wheel.png");
    img->FromPad(canv5);
    img->WriteImage(dir+"/compare_dxy.png");
    img->FromPad(canv6);
    img->WriteImage(dir+"/compare_TA1.png");
    img->FromPad(canv7);
    img->WriteImage(dir+"/compare_TA2.png");
    img->FromPad(canv8);
    img->WriteImage(dir+"/compare_TA3.png");
    img->FromPad(canv9);
    img->WriteImage(dir+"/compare_TA4.png");
...
{
  if (data_xs == NULL)
    std::cout << "data pointer to the TH1 is NULL" << std::endl;
  if (mc_xs == NULL)
    std::cout << "emu pointer to the TH1 is NULL" << std::endl;

  TCanvas *data_mc_canvas = new TCanvas("compare_"+canvas, "compare_"+canvas, 550, 600);
  ...
  data_xs->GetYaxis()->SetTitle("Muons");
  data_xs->GetYaxis()->SetTitleOffset(1.2);
  ...
  TLegend *leg;
  leg = new TLegend(0.75, 0.7, 0.95, 0.95, "CMS Internal | Run "+RUN);
  //  leg->SetHeader("CMS Internal | Run "+RUN, "C");
